...
void DateToS(WENSHIDU *Time,float datax,float datax1);
...
{ 
	...
	else
	{
		...
		//DateToStr(&S,temp_val.f,humi_val.f);
                DateToS(&S,temp_val.f,humi_val.f);
	}
}
...
void DateToS(WENSHIDU *Time,float datax,float datax1)
{
    ...
    i=datax*10;
	Time->DateString1[0] = i>>8;
    	Time->DateString1[1] = i&0xff;

    i=datax1*10;
	Time->DateString1[2] = i>>8;
        Time->DateString1[3] = i&0xff;
