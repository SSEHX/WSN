...
static void sendDummyReport(void)
{
  ...
  pData[SENSOR_DATA_OFFSET+0]=0X02;     //逻辑类型
  pData[SENSOR_DATA_OFFSET+1]=LO_UINT16(soradr);     //传感器编号低位
  pData[SENSOR_DATA_OFFSET+2]=HI_UINT16(soradr);     //传感器编号高位
  pData[SENSOR_DATA_OFFSET+3]=sortype;     // 传感器类型
  
  readsensor(sortype,&val1,&val2);
  ...
  zb_SendDataRequest( 0xFFFE, DUMMY_REPORT_CMD_ID, SENSOR_LENGTH+4, pData, 0, txOptions, 0 );
