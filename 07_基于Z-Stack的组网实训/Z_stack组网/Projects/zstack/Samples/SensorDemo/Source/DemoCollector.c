...
static void sendGtwReport(gtwData_t *gtwData ,uint8 *pData, uint8 len);
...
{ 
  ...
  // Send gateway report
  sendGtwReport(&gtwData, pData,len);
}
  ...
  if ( event != HAL_UART_TX_EMPTY ) 
  {
    ...
    if ( len>0 ) 
    {
      ...
      if ((pBuf[0]==0xff)&& (pBuf[1]==0xf5))
	    ...
	    if ( ++reportNr1<ACK_REQ_INTERVAL && reportFailureNr==0 ) 
	    {
	    ...
	    zb_SendDataRequest( 0xFFFF, DUMMY_REPORT_CMD_ID , len,pBuf, 0, txOptions, 0 );
...
static void sendGtwReport(gtwData_t *gtwData ,uint8 *pData, uint8 len)
{
  ...
  // Length
  pFrame[FRAME_LENGTH_OFFSET] = len+6;
  ...
  // Length
  pFrame[FRAME_DATA_OFFSET+ ZB_RECV_LEN_OFFSET] = len; 
  pFrame[FRAME_DATA_OFFSET+ ZB_RECV_LEN_OFFSET+ 1] = 0;
  
  // Data
  osal_memcpy(&pFrame[FRAME_DATA_OFFSET+ ZB_RECV_DATA_OFFSET],pData, len);
  ...
  // Frame Check Sequence
  pFrame[plen - 1] = calcFCS(&pFrame[FRAME_LENGTH_OFFSET], (plen - 2) );
  
  // Write report to UART
  HalUARTWrite(HAL_UART_PORT_0,pFrame, plen);
}
