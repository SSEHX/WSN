 ...
 *
 * @brief        接收到ZIGBEE的信息处理
                The SAPI_ReceiveDataIndication callback function is called
...
void SAPI_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData  )
{
  ...
  // 继电器命令处理
  uint8   flag;
    //FF F5 05 01 12 34 55 AA LRC 命令的应答为：FF F5 06 01 12 34 55 AA FF [LRC]：
    #if defined (DEBUG_DATA)
    uart_printf("接收数据data:");
    uart_datas(pData, len);
    #endif 
  if( (*pData==0xff)&& (*(pData+1)==0xf5) )
#ifndef  NO_LRC    
    if( *(pData+len-1)==lrc_checksum(pData,len-1) )
#endif        
      if( (*(pData+5)==serh)&& (*(pData+4)==serl) )    
      {
        osal_memcpy(BUF,pData,32);      
        if( *(pData+3) == 0X01)
        {    // 数据校验
            BUF[2]++; 
            BUF[len-1]=BUF[len-2]^  BUF[len-3] ;
            BUF[len]=lrc_checksum(BUF,len);
            HAL_TOGGLE_LED1();
            osal_set_event( sapi_TaskID ,REPLAY_EVENT ) ; 
            return;
        }
        BUF[len-1]=00;
        BUF[len]=00;
        ...
        {    // 继电器命令
            ...
            BUF[2]++;
            BUF[2]++;
            BUF[len+1]=lrc_checksum(BUF,len+1);
            HAL_TOGGLE_LED1();
            osal_set_event(sapi_TaskID ,REPLAY_EVENT ) ;        
         }  
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:         /*zigbee 接收成功*/
  ...
  if ( events & ( ZB_TIME_EVENT ) )
  {
    ...
    test_read_flash();
      ...
      if(!rdcfg)
      {
        ...
        uart_printf("\r\nZigbee start!\r\n");
        //MicroWait (10000);
        uart_printf("节点类型（0-协调器,1-路由,2-全功能节点,3-终端节点,4-继电器): %d\r\n", LOG_TYPE);
       
        uart_printf("Pand_id: 0X%04x\r\n", BUILD_UINT16(buf[0],buf[1]));
        
        uart_printf("chancel: %d\r\n", buf[2]);
        
#if ((LOG_TYPE!=00) && (LOG_TYPE!=01))
        {
        uart_printf("设备号: 0X%04x\r\n", BUILD_UINT16(serl,serh));
        uart_printf("类型: 0X%02x\r\n", Dtype);
        }
#endif
        
      }
      
      osal_start_timerEx( sapi_TaskID, ZB_TIME_EVENT,100 );
      uartRxCB( HAL_UART_PORT_0,0 );
      return (events ^ ZB_TIME_EVENT );
  }
  
  if ( events & ( ZB_RST_EVENT ) )
  {
      zb_SystemReset();
  }
...
#if (LOG_TYPE==4)
  if ( events & REPLAY_EVENT)
    {
    #if  defined (DEBUG_DATA)
        uart_printf("RLYdataS:");
        uart_datas(BUF, BUF[2]+4);
        uart_printf("\r\n");
    #endif   
        sendreplay( BUF, BUF[2]+4);
...
#endif
  osal_start_timerEx( sapi_TaskID, ZB_TIME_EVENT,1000 );
}
/*********************************************************************
 * @fn      SAPI_SendCback
 *
