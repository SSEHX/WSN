/**************************************************************************************************
  Filename:       ZDApp.c
  Revised:        $Date: 2010-01-04 12:16:55 -0800 (Mon, 04 Jan 2010) $
  Revision:       $Revision: 21428 $
...
// Size of the ZDO callback function table zdoCBFunc[]
#define MAX_ZDO_CB_FUNC 1
...
uint8 ZDOInitDevice( uint16 startDelay )
{
  ...
  else
  {
   ...
   networkStateNV = ZDO_INITDEV_NEW_NETWORK_STATE;  //++by cl
...
{
  (void)newAddr;  // remove if this parameter is used
 ...
 * @return      void 
 */
void ZDO_ConcentratorIndicationCB( uint16 nwkAddr )
{
  // Write your code here to do something with the concentrator's
  // address.
  (void)nwkAddr;
}
...
void ZDO_SyncIndicationCB( uint8 type, uint16 shortAddr )
{
  ...
  if ( ZSTACK_END_DEVICE_BUILD
    || (ZSTACK_ROUTER_BUILD && ((_NIB.CapabilityInfo & ZMAC_ASSOC_CAPINFO_FFD_TYPE) == 0)))
  {
    if ( type == 1 )
    {
