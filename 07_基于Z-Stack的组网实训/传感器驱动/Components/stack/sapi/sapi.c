...
uint8 flag_write_num_c = 0;
uint8 flag_write_num_r = 0;
...
void zb_StartRequest()
{
	...
	if ((logicalType >   ZG_DEVICETYPE_ENDDEVICE) ||
		#if !ZG_BUILD_ENDDEVICE_TYPE                                  // Only RTR or Coord possible.   ZG_BUILD_ENDDEVICE_TYPE == 0x04
			(logicalType == ZG_DEVICETYPE_ENDDEVICE) ||
		#endif
		#if !ZG_BUILD_RTR_TYPE                                               // Only End Device possible.    ZG_BUILD_RTR_TYPE  == 0x03
			(logicalType == ZG_DEVICETYPE_ROUTER) ||
			(logicalType == ZG_DEVICETYPE_COORDINATOR) ||
		#elif ZG_BUILD_RTRONLY_TYPE                                       // Only RTR possible.           ZG_BUILD_RTRONLY_TYPE == 0x00
			(logicalType == ZG_DEVICETYPE_COORDINATOR) ||
		#elif !ZG_BUILD_JOINING_TYPE                                      // Only Coord possible.        ZG_BUILD_JOINING_TYPE   == 0x06
			(logicalType == ZG_DEVICETYPE_ROUTER) ||
		#endif
	    	(0))
	{
   		...
   		if (sapi_bindInProgress == 0xffff)
    		{
        			...
        			else if ( ZDO_AnyClusterMatches( 1, &commandId, sapi_epDesc.simpleDesc->AppNumInClusters, sapi_epDesc.simpleDesc->pAppInClusterList ) )
				{
					...
					// Set a timer to make sure bind completes
		          		...
		          		osal_start_timerEx(sapi_TaskID, ZB_BIND_TIMER,AIB_MaxBindingTime); //AIB_MaxBindingTime = 16000,  every 16s trigger the "ZB_BIND_TIMER" event
					#else
...
UINT16 SAPI_ProcessEvent( byte task_id, UINT16 events )
{
		...
		while ( pMsg )
		{
			switch ( pMsg->event )
			{
				...
				case ZDO_STATE_CHANGE:
					...
					if (pMsg->status == DEV_END_DEVICE || pMsg->status == DEV_ROUTER || pMsg->status == DEV_ZB_COORD )
					{
	...
	if ( events & ZB_ALLOW_BIND_TIMER ) //0x4000
	{
		afSetMatch(sapi_epDesc.simpleDesc->EndPoint, FALSE);
	...
	if ( events & ZB_BIND_TIMER )  //0x2000
	{
	...
	if ( events & ( ZB_USER_EVENTS ) )  //0x00FF
	{
...
void SAPI_Init( byte task_id )
{
	...
	//osal_set_event(task_id, MY_SHT10_EVT);
