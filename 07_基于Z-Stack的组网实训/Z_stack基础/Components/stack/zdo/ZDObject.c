/**************************************************************************************************
  Filename:       ZDObject.c
  Revised:        $Date: 2009-03-19 11:37:20 -0700 (Thu, 19 Mar 2009) $
  Revision:       $Revision: 19466 $
...
void ZDO_StartDevice( byte logicalType, devStartModes_t startMode, byte beaconOrder, byte superframeOrder )
{
  ...
  if ( ZG_BUILD_COORDINATOR_TYPE && logicalType == NODETYPE_COORDINATOR )
  {
    ...
    else
    {
      ...
      HalLcdWriteString( "StartDevice ERR", HAL_LCD_LINE_4 );
      HalLcdWriteString( "MODE unknow", HAL_LCD_LINE_5 );
#endif
    }
  }
  ...
  if ( ZG_BUILD_JOINING_TYPE && (logicalType == NODETYPE_ROUTER || logicalType == NODETYPE_DEVICE) )
  {
    ...
    else
    {
      ...
      //HalLcdWriteScreen( "StartDevice ERR", "MODE unknown" );
      HalLcdWriteString("StartDevice ERR", HAL_LCD_LINE_4 );
      HalLcdWriteString("MODE unknown", HAL_LCD_LINE_5 );
#endif
    }
  }
  ...
  while ( epDesc )
  {
    if ( epDesc->epDesc->endPoint != ZDO_EP )
    {
      ...
      if ( msgPtr )
      {
        ...
        osal_msg_send( *(epDesc->epDesc->task_id), (uint8 *)msgPtr );
      }
    }
...
#if defined( LCD_SUPPORTED )
  HalLcdWriteString( "End Device Bind", HAL_LCD_LINE_4 );
  if ( Status == ZDP_SUCCESS )
    HalLcdWriteString( "Success Sent", HAL_LCD_LINE_5 );
  else
    HalLcdWriteString( "Timeout", HAL_LCD_LINE_5 );
#endif
...
void ZDO_ProcessMatchDescReq( zdoIncomingMsg_t *inMsg )
{
  ...
  if ( epCnt )
  {
              ...
              ZDAppNwkAddr.addr.shortAddr, epCnt, (uint8 *)ZDOBuildBuf, inMsg->SecurityUse ) )
    {
      ...
      HalLcdWriteString("Match Desc Req", HAL_LCD_LINE_4 );
      HalLcdWriteString("Rsp Sent", HAL_LCD_LINE_5 );
#endif
    }
  }
  else
  {
    ...
    HalLcdWriteString("Match Desc Req", HAL_LCD_LINE_4 );
    HalLcdWriteString("Non Matched", HAL_LCD_LINE_5 );
#endif
  }
