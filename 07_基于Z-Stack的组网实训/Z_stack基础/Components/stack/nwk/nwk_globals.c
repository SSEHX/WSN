...
void nwk_Status( uint16 statusCode, uint16 statusValue )
{
  ...
  switch ( statusCode )
  {
    case NWK_STATUS_COORD_ADDR:
      if ( ZSTACK_ROUTER_BUILD )
      {
        HalLcdWriteString( (char*)ZigbeeCoordStr, HAL_LCD_LINE_4 );
        HalLcdWriteStringValue( (char*)NetworkIDStr, statusValue, 16, HAL_LCD_LINE_5 );
    ...
    case NWK_STATUS_ROUTER_ADDR:
      if ( ZSTACK_ROUTER_BUILD )
      {
        HalLcdWriteStringValue( (char*)RouterStr, statusValue, 16, HAL_LCD_LINE_4 );
      }
      break;
      ...
      if ( ZSTACK_ROUTER_BUILD )
      {
        if ( statusValue == ZSuccess )
        {
          //HalLcdWriteScreen( (char*)OrphanRspStr, (char*)SentStr );
           HalLcdWriteString( (char*)OrphanRspStr, HAL_LCD_LINE_4 );
           HalLcdWriteString( (char*)SentStr, HAL_LCD_LINE_5 );
        }
        else
        {
          //HalLcdWriteScreen( (char*)OrphanRspStr, (char*)FailedStr );
          HalLcdWriteString( (char*)OrphanRspStr, HAL_LCD_LINE_4 );
          HalLcdWriteString( (char*)FailedStr, HAL_LCD_LINE_5 );
        }
      }
      break;
    ...
    case NWK_ERROR_ASSOC_RSP:
      if ( ZSTACK_ROUTER_BUILD )
      {
        HalLcdWriteString( (char*)AssocRspFailStr, HAL_LCD_LINE_4 );
        HalLcdWriteValue( (uint32)(statusValue), 16, HAL_LCD_LINE_5 );
      }
      break;
    ...
    case NWK_STATUS_ED_ADDR:
      ...
      {
        HalLcdWriteStringValue( (char*)EndDeviceStr, statusValue, 16, HAL_LCD_LINE_4 );
      }
      break;

    case NWK_STATUS_PARENT_ADDR:
      HalLcdWriteStringValue( (char*)ParentStr, statusValue, 16, HAL_LCD_LINE_5 );
      break;

    case NWK_STATUS_ASSOC_CNF:
      //HalLcdWriteScreen( (char*)AssocCnfStr, (char*)SuccessStr );
      HalLcdWriteString((char*)AssocCnfStr, HAL_LCD_LINE_4 );
      HalLcdWriteString((char*)SuccessStr, HAL_LCD_LINE_5 );
      break;

    case NWK_ERROR_ASSOC_CNF_DENIED:
      HalLcdWriteString((char*)AssocCnfFailStr, HAL_LCD_LINE_4 );
      HalLcdWriteValue( (uint32)(statusValue), 16, HAL_LCD_LINE_5 );
      break;

    case NWK_ERROR_ENERGY_SCAN_FAILED:
      //HalLcdWriteScreen( (char*)EnergyLevelStr, (char*)ScanFailedStr );
      HalLcdWriteString((char*)EnergyLevelStr, HAL_LCD_LINE_4 );
      HalLcdWriteString((char*)ScanFailedStr, HAL_LCD_LINE_5 );
      break;
  }
#endif
}
