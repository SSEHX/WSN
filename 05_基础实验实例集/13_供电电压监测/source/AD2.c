...
/*********************************************************************
 * 函数名称：getVol
 * 功    能：进行AD 转换，将得到的结果求均值后将 AD 结果转换为电压返回
 * 入口参数：无
 * 出口参数：无
 * 返 回 值：无
 ********************************************************************/
...
{
   signed short value;
   ADCCON3 = (0x3F);       //选择内部1.15v为参考电压；12位分辨率；对AVDD5/3采样
   ...
   value |= ((int)ADCH << 6);     //8位转为16为，后补6个0，取得最终转化结果，存入value中
    // 若value<0，就认为它为0 /
    if(value < 0) value = 0;  
    return ((value * 1.15) /8192); ;   //根据公式计算出AVDD5/3值*/
  }
...
{  
  ...
  /*********以下代码采集AVDD5/3值并处理**********/ 
   float tempVol;
   char output[]="";
   UART0SendString("测试 CC2530的电源电压!\r\n");
    while(1)
    {
